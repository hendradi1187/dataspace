...
export const ConnectorDetail = () => {
  ...
  const {
    ...
    error,
    isEditing,
    handleSave: saveConnector,
    toggleEditMode,
  } = useDetailData(
    id || '',
    (id) => connectorsService.get(id),
    (id, data) => connectorsService.update(id, data),
    undefined,
    {
      onSuccess: () =>
        addNotification({
  ...
  // Initialize formData when connector loads
  if (!formData.id && connector) {
