...
export const Vocabularies = () => {
  ...
  const {
    ...
    handleSort,
    refetch,
  } = useListData((params) => vocabulariesService.list(params), {
    pageSize: 10,
  });
  ...
  const columns = [
    { key: 'name' as const, label: 'Vocabulary Name', sortable: true },
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:justify-between md:items-center">
        <div>
          <h1 className="text-3xl font-bold text-neutral-900">Vocabularies</h1>
          <p className="text-neutral-600 mt-2">Manage shared vocabularies and terms</p>
        </div>
        <Button icon={<Plus size={16} />} onClick={openCreateModal}>Create Vocabulary</Button>
      </div>
      <DataTable<Vocabulary>
        ...
        sortOrder={sortOrder}
        onRowClick={(vocab) => {
          window.location.href = `/vocabularies/${vocab.id}`;
        }}
      />
